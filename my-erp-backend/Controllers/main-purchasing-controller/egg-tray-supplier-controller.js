...
class EggTraySupplierController {
    constructor(dbConnection) {
        this.db = dbConnection;
    }

    async getAllSuppliers(search = '') {
        let query = 'SELECT * FROM egg_tray_suppliers';
        const values = [];
        let counter = 1;
    ...
    async getSupplierById(id) {
        const query = 'SELECT * FROM egg_tray_suppliers WHERE id = $1';
        const result = await this.db.query(query, [id]);
        return result.rows[0];
    }

    async getSupplierByCode(supplierId) {
        const query = 'SELECT * FROM egg_tray_suppliers WHERE supplier_id = $1';
    ...
    async addSupplier(supplierData) {
        ...
        const query = `
            INSERT INTO egg_tray_suppliers 
    ...
    async deleteSupplier(supplierId) {
        const query = 'DELETE FROM egg_tray_suppliers WHERE supplier_id = $1';
        const result = await this.db.query(query, [supplierId]);
...
module.exports = EggTraySupplierController;
