...
class FeedsSupplierController {
    constructor(dbConnection) {
        this.db = dbConnection;
    }

    async getAllSuppliers(search = '') {
        let query = 'SELECT * FROM feeds_suppliers';
        const values = [];
        let counter = 1;
    ...
    async getSupplierById(id) {
        const query = 'SELECT * FROM feeds_suppliers WHERE id = $1';
        const result = await this.db.query(query, [id]);
        return result.rows[0];
    }

    async getSupplierByCode(supplierId) {
        const query = 'SELECT * FROM feeds_suppliers WHERE supplier_id = $1';
    ...
    async deleteSupplier(supplierId) {
        const query = 'DELETE FROM feeds_suppliers WHERE supplier_id = $1';
        const result = await this.db.query(query, [supplierId]);
        return result.rowCount > 0;
    }

    async getNextSupplierId() {
        const query = "SELECT MAX(CAST(SUBSTRING(supplier_id FROM '\\d+') AS INTEGER)) as max_num FROM feeds_suppliers";
        const result = await this.db.query(query);
        const maxNum = result.rows[0]?.max_num || 0;
        return 'FeSuID-' + (maxNum + 1);
    }
}

module.exports = FeedsSupplierController;
